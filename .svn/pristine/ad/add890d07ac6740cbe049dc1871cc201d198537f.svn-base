<%@ Page Language="C#" MasterPageFile="~/MasterPage.master" AutoEventWireup="true" CodeFile="listTakeOut.aspx.cs" Inherits="main_visit_listVisit" Title="Chi tiết đăng ký (신청 내역)" %>
...
<tr>
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		<tr>
			...
			<td class="contents_title">Chi tiết đăng ký tài sản ra cổng (반출 신청 내역)</td>
			<td align="right" class="location">HOME &gt;Chi tiết đăng ký tài sản ra cổng (반출 신청 내역)</td>
		</tr>
		<tr>
	...
	<td valign="top">			
		...
		<tr>
			<td align="right">
                ...
                <label>From</label>
				<input type="text" size="8" name="searchStartDate" value="<%=Request["searchStartDate"]%>" data-datepicker data-format="yyyy-MM-DD" /> 
			<%--	<img src="../../images/icon/calendar.gif" alt="Lựa chọn ngày (날짜선택)" style="cursor:hand" align="absmiddle"  onclick="popUpCalendar(this, document.forms[0].searchStartDate,'yyyy-mm-dd',-1,-1);" />--%>
			      <label>To</label>
            	<input type="text" size="8" name="searchEndDate" value="<%=Request["searchEndDate"]%>" data-datepicker data-format="yyyy-MM-DD" /> 
				...
				<asp:Button ID="Button2" runat="server" Text="Tìm kiếm (검색)" />
                    <asp:Button ID="Button1" runat="server" Text="Tải (다운로드)" OnClick="Button1_Click" />
		...
		<FooterStyle BackColor="White" ForeColor="#000066" />
                    <Columns>
						...
						<asp:BoundField DataField="TakeOutDataCode" HeaderText="Tình trạng (상태)" >
						    <HeaderStyle HorizontalAlign="Center" />
						    <ItemStyle HorizontalAlign="Center" />
						</asp:BoundField>
